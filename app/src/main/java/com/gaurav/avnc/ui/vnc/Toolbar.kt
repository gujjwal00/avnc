...
class Toolbar(private val activity: VncActivity) {
    ...
    private fun setupOpenerButton() {
        var verticalBias = viewModel.pref.runInfo.toolbarOpenerBtnVerticalBias
        val listener = object : GestureDetector.SimpleOnGestureListener() {
            override fun onDown(e: MotionEvent) = true
            override fun onSingleTapConfirmed(e: MotionEvent) = openerButton.performClick()

            override fun onScroll(e1: MotionEvent?, e2: MotionEvent, distanceX: Float, distanceY: Float): Boolean {
                val parentHeight = (openerButton.parent as View).height
        ...
        openerButton.setOnTouchListener { _, e ->
            if (e.actionMasked == MotionEvent.ACTION_UP && verticalBias != viewModel.pref.runInfo.toolbarOpenerBtnVerticalBias)
                viewModel.pref.runInfo.toolbarOpenerBtnVerticalBias = verticalBias
            gestureDetector.onTouchEvent(e)
        }
        drawerLayout.addOnLayoutChangeListener { _, _, _, _, _, _, _, _, _ ->
            updateOpenerBtnPosition(openerButton, verticalBias)
        }
